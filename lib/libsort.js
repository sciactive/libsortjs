...
function zipsort(arr, options, recursing) {
  ...
  var oneThird = Math.floor(length / 3);
  var twoThirds = Math.floor(length * 2 / 3);
  do {
    do {
      swapped = false;
      for (var i = 0; i <= mid; i++) {
  ...
  if (!recursing) {
    for (var _i3 = offset + 1; _i3 < offset + length; _i3++) {
      var k = _i3;
...
function cocktailshakersort(arr, options) {
  ...
  do {
    ...
    for (var _i4 = m - 1; _i4 >= nLeft; _i4--) {
      if (compareFunction(arr[_i4], arr[_i4 + 1]) > 0) {
        swaps = true;
        swapFunction(arr, _i4, _i4 + 1);
        m = _i4;
      }
    }
    if (!swaps) {
