...
function zipsort(arr, options, recursing) {
  ...
  do {
    ...
    for (var o = 0; o < denom; o++) {
      // Sort parts of the list.
      var oOffset = offset + fraction * o;
      var oLength = Math.min(fraction, length - (oOffset - offset));
      if (oLength > 1) {
        swapped = zipsort(arr, { offset: oOffset, length: oLength, compareFunction: compareFunction, swapFunction: swapFunction, sortedCallbackFunction: sortedCallbackFunction }, true) && swapped;
      }
    }
    denom++;
